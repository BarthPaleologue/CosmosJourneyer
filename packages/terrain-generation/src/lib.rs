...
static mut TERRAIN_FUNCTION: Option<Box<TerrainFunction>> = None;
static mut CURRENT_PLANET_SEED: f32 = 0.0;
...
pub fn build_chunk_vertex_data(
    ...
    unsafe {
        if data.planet_seed != CURRENT_PLANET_SEED {
            CURRENT_PLANET_SEED = data.planet_seed;
            TERRAIN_FUNCTION = Some(make_terrain_function(data.terrain_settings));
        }
    }
    ...
    for x in 0..nb_vertices_per_row {
        for y in 0..nb_vertices_per_row {
            ...
            unsafe {
                TERRAIN_FUNCTION.as_ref().unwrap()(
                    &unit_sphere_coords,
                    seed,
                    &mut vertex_position,
                    &mut vertex_gradient,
