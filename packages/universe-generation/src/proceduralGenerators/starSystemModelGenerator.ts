...
import { SeedHalfRange } from "#/constants";
...
export function generateStarSystemModel(
    ...
    planets.forEach((planet) => {
        ...
        for (let j = 0; j < nbMoons; j++) {
            ...
            const satelliteSeed = centeredRand(planetRng, GenerationSteps.MOONS + j) * SeedHalfRange;
    ...
    for (let i = 0; i < nbAnomalies; i++) {
        const anomalySeed = centeredRand(systemRng, GenerationSteps.ANOMALIES + i * 100) * SeedHalfRange;
    ...
    if (isCivilized) {
        ...
        planetsWithStations.forEach((planet) => {
            const spaceStationSeed =
                centeredRand(systemRng, GenerationSteps.SPACE_STATIONS + orbitalFacilities.length) * SeedHalfRange;
