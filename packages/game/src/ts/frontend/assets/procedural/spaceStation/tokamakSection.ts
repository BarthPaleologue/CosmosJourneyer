...
export class TokamakSection implements Transformable {
    ...
    constructor(requiredNetEnergyOutput: number, assets: RenderingAssets, scene: Scene) {
        ...
        const minorRadius = Math.cbrt(plasmaVolume / (Math.PI ** 2 * aspectRatio * elongation));
        const majorRadius = aspectRatio * minorRadius;

        const tokamak = createRing(majorRadius, 2 * minorRadius, 2 * minorRadius * elongation, 32, scene);
        tokamak.convertToFlatShadedMesh();
        tokamak.parent = this.getTransform();
        tokamak.material = assets.materials.metalSection;
