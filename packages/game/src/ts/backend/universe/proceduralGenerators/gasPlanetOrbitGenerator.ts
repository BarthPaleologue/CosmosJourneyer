...
/**
 * Samples a plausible semi-major axis for a gas giant.
 *
 * Model:
 *  - Most giants form beyond the snow line (scaled with sqrt(L)).
 *  - Optional rare hot-Jupiter branch (migration).
 *
...
export function getGasPlanetOrbitRadius(stellarTemperature: number, stellarRadius: number, rng: () => number): number {
    ...
    const safeRelLum = Math.max(relativeLuminosity, 0);

    // Empirical Solar System-calibrated snow line heuristic
    const snowLine = astronomicalUnitToMeters(2.7) * Math.sqrt(safeRelLum);

    // --- Optional: rare hot-Jupiter branch (observationally ~percent-level around G stars) ---
    // Keep this small unless you explicitly want a Hot Jupiter-heavy universe.
    ...
    // --- Main branch: cold/warm giants beyond the snow line ---
    // Jupiter is ~1.9× snow line; Saturn ~3.5× snow line (Solar System reference point).
    // Sample a broad range so you can still get "warm" giants.
    const innerFactor = 1.3 + 0.9 * rng(); // ~[1.3, 2.2] × snowLine
    const outerFactor = 5.0 + 10.0 * rng(); // ~[5, 15] × snowLine
    ...
    // For large planets, use uniform-in-sqrt(a) to weight slightly more toward wider orbits
    // (swap to log-uniform if you prefer more close-in giants).
    const u = rng();
