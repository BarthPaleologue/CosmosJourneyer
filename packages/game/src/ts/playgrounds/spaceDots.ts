...
export function createSpaceDotsScene(
    ...
    const carrier = new TransformNode("spaceDotsCarrier", scene);
    carrier.rotationQuaternion = Quaternion.Identity();

    const spaceDots = new SpaceDots(scene);
    spaceDots.getTransform().parent = carrier;
    spaceDots.setThrottle(DEFAULT_THROTTLE);
    spaceDots.setSteering(DEFAULT_YAW, DEFAULT_PITCH);
    const controls = createControlPanel(spaceDots, carrier);
...
function createControlPanel(
...
): { panel: HTMLDivElement; isPaused: () => boolean } {
    let paused = false;
    let yaw = DEFAULT_YAW;
    ...
    panel.appendChild(
        createSlider("Yaw bend", -1, 1, 0.01, DEFAULT_YAW, (value) => {
            yaw = value;
            spaceDots.setSteering(yaw, pitch);
            return `yaw = ${value.toFixed(2)}`;
        }),
    );

    panel.appendChild(
        createSlider("Pitch bend", -1, 1, 0.01, DEFAULT_PITCH, (value) => {
            pitch = value;
            spaceDots.setSteering(yaw, pitch);
            return `pitch = ${value.toFixed(2)}`;
        }),
    );

    panel.appendChild(
        createSlider("Ship roll", -180, 180, 1, DEFAULT_ROLL, (value) => {
            carrier.rotationQuaternion = Quaternion.FromEulerAngles(0, 0, (value * Math.PI) / 180);
            return `roll = ${value.toFixed(0)} deg`;
        }),
    );
