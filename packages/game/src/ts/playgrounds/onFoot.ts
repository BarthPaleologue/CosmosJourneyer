...
export async function createOnFootScene(
    ...
    const rover = createWolfMk2(assets, scene, new Vector3(-16, 3, 16), { axis: Vector3.Up(), angle: 0.5 });
    if (!rover.success) {
    ...
    roverControls.setVehicle(rover.value);
    ...
    interactionSystem.register({
        getPhysicsAggregate: () => rover.value.frame,
        getInteractions: () => [
            {
                label: "Drive",
                perform: async () => {
