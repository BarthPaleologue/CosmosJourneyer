...
new Slider("atmosphereRadius", document.getElementById("atmosphereRadius"), 0, 40, 10, (val) => {
    postProcess.modifiers.atmosphereRadiusModifier = val / 10;
});
new Slider("betaRayleigh", document.getElementById("betaRayleigh"), 0, 40, 10, (val) => {
    postProcess.modifiers.intensityModifier = val / 10;
});
new Slider("falloff", document.getElementById("falloff"), 0, 30, 10, (val) => {
    postProcess.modifiers.falloffModifier = Math.pow(val, 5.4) / 10;
});
new Slider("maxHeight", document.getElementById("maxHeight"), 0, 30, 10, (val) => {
    postProcess.modifiers.maxHeightModifier = Math.pow(val, 1000) / 10;
});
new Slider("rayleighScale", document.getElementById("rayleighScale"), 0, 30, 10, (val) => {
    postProcess.modifiers.rayleighScaleModifier = val / 10;
});
new Slider("mieScale", document.getElementById("mieScale"), 0, 30, 10, (val) => {
    postProcess.modifiers.mieScaleModifier = Math.pow(val, 1.5) / 10;
});
let keyboard = {};
