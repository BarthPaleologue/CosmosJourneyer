...
scene.executeWhenReady(() => {
    ...
    engine.runRenderLoop(() => {
        ...
        let speed = 0.01;
        if (keyboard["z"])
            freeCamera.position.addInPlace(forward.scale(speed * engine.getDeltaTime()));
        if (keyboard["s"])
            freeCamera.position.subtractInPlace(forward.scale(speed * engine.getDeltaTime()));
        if (keyboard["q"])
            freeCamera.position.subtractInPlace(right.scale(speed * engine.getDeltaTime()));
        if (keyboard["d"])
            freeCamera.position.addInPlace(right.scale(speed * engine.getDeltaTime()));
        if (keyboard[" "])
            freeCamera.position.addInPlace(upward.scale(speed * engine.getDeltaTime()));
        if (keyboard["Shift"])
            freeCamera.position.subtractInPlace(upward.scale(speed * engine.getDeltaTime()));
        scene.render();
    });
});
