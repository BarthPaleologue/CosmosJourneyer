...
vec3 ocean(vec3 originalColor, vec3 rayOrigin, vec3 rayDir, float maximumDistance) {
    ...
    vec3 normal = normalize(samplePoint - planetPosition);
    normal = triplanarNormal(samplePoint, normal, normalMap, 0.001, 0.1, 1.0);
    
    vec3 sunDir = normalize(sunPosition - samplePoint); // direction to the light source

    float ndl = pow(max(0.0, dot(normal, sunDir)), 0.5); // dimming factor due to light inclination relative to vertex normal in world space
    ...
    float diffuseLighting = clamp(dot(normal, sunDir), 0.0, 1.0);
    ...
    if(distanceThroughOcean > 0.0) {
        ...
        oceanColor += specularHighLight * specularPower;

        return lerp(originalColor, oceanColor, alpha) * ndl;
    } else {
