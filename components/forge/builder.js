...
function colorFunction(p, n) {
    ...
    if (position.lengthSquared() > Math.pow((radius * (1 + colorSettings.iceCapThreshold / 100 - Math.pow(positionN.y, 20))), 2)) {
        ...
        if (dot > colorSettings.snowSteepDotLimit)
            ...
            color = colorSettings.steepColor;
    }
    else {
        // if lower region
        if (dot < colorSettings.plainSteepDotLimit)
            color = colorSettings.steepColor;
        else {
            if (position.lengthSquared() > Math.pow((colorSettings.waterLevel / 2 + radius), 2))
                color = colorSettings.plainColor;
            else if (position.lengthSquared() > Math.pow(((colorSettings.waterLevel / 2 + radius) * 0.99), 2))
                color = colorSettings.sandColor;
            else
                color = colorSettings.steepColor;
        }
    }
