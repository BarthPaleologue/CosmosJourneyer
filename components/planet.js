...
export class Planet extends ProceduralSphere {
    constructor(_id, _radius, _position, _nbSubdivisions, _maxDepth, _scene) {
        ...
        let terrainFunction = function (p) {
            ...
            let moutains = 0;
            if (continents > 95 * seuil * noiseStrength)
                moutains = 40 * noiseStrength * noiseEngine.normalizedSimplex3FromVector(coords.scale(noiseFrequency * 20));
            let erosion = 0;
            if (continents > 0.5 * seuil * noiseStrength)
                erosion = 2 * noiseStrength * noiseEngine.simplex3FromVector(coords.scale(noiseFrequency * 100));
