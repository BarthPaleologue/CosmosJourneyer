...
export class ProceduralEngine {
    ...
    static createSphereChunk(radius: number, size: number, subs: number, position: BABYLON.Vector3, offset: BABYLON.Vector3, direction: Direction, scene: BABYLON.Scene, terrainFunction: (p: BABYLON.Vector3) => BABYLON.Vector3, parentPosition = BABYLON.Vector3.Zero()): [BABYLON.Mesh, BABYLON.Vector3] {
        let vertices = [];
        let faces: number[][] = [];
        let nbSubdivisions = subs + 1;
