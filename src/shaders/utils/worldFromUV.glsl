...
vec3 worldFromUV(vec2 pos, mat4 inverseProjection, mat4 inverseView) {
    ...
    vec4 posWS = inverseView * vec4(posVS.xyz, 1.0); // then we use inverse view to get to world space, division by w to get actual coordinates
    return  posWS.xyz / posWS.w; // the coordinates in world space
}