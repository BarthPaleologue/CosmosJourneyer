...
vec4 oceanColor(vec4 originalColor, vec3 rayOrigin, vec3 rayDir, float maximumDistance) {
    ...
    if (!(rayIntersectSphere(rayOrigin, rayDir, object_position, actualRadius, impactPoint, escapePoint))) {
    ...
    for (int i = 0; i < nbStars; i++) {
        ...
        if (length(rayOrigin - object_position) > ocean_radius) {
            // if above ocean surface then specular highlight
            finalColor += computeSpecularHighlight(sunDir, rayDir, normalWave, ocean_smoothness, ocean_specularPower) * star_colors[i];
        }
    }
...
void main() {
    ...
    vec4 finalColor = oceanColor(screenColor, camera_position, rayDir, maximumDistance);
