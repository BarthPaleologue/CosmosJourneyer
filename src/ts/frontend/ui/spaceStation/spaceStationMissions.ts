 ...
 */
export async function generateMissionsDom(
    stationModel: DeepReadonly<OrbitalFacilityModel>,
    player: Player,
    universeBackend: IUniverseBackend,
    soundPlayer: ISoundPlayer,
): Promise<HTMLDivElement> {
    const starSystemModel = await universeBackend.getSystemModelFromCoordinates(stationModel.starSystemCoordinates);
    if (starSystemModel === null) {
        throw new Error("Cannot generate missions for a space station in an unknown star system");
    }

    const sightSeeingMissions = await generateSightseeingMissions(
        stationModel,
        starSystemModel,
        universeBackend,
        player,
    ...
    for (const neighbor of neighborSystems) {
        const systemModel = await universeBackend.getSystemModelFromCoordinates(neighbor.coordinates);
        if (systemModel === null) {
    ...
    for (const mission of sightSeeingMissions) {
