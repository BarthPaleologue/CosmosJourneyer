...
export class StarSystemView implements View {
    ...
    constructor(
        ...
        StarSystemInputs.map.jumpToSystem.on("complete", async () => {
            ...
            const currentSystemPosition = this.universeBackend.getSystemGalacticPosition(
                this.getStarSystem().model.coordinates,
            );
            const targetSystemPosition = this.universeBackend.getSystemGalacticPosition(target.systemCoordinates);
            ...
            const systemModel = await this.universeBackend.getSystemModelFromCoordinates(starSystemCoordinates);
            if (systemModel === null) {
     ...
     */
    public async initStarSystem(): Promise<void> {
        ...
        if (previousSystem === undefined) {
            ...
            const currentSystemPosition = this.universeBackend.getSystemGalacticPosition(starSystem.model.coordinates);
            const previousSystemPosition = this.universeBackend.getSystemGalacticPosition(previousSystem);
        ...
        for (const neighbor of getNeighborStarSystemCoordinates(
            ...
            const systemTarget = await this.getStarSystem().addSystemTarget(neighbor.coordinates, this.universeBackend);
        ...
        if (this.player.currentItinerary !== null) {
            ...
            if (starSystemCoordinatesEquals(starSystem.model.coordinates, targetCoordinates)) {
                ...
                if (newItinerary.success) {
                    ...
                    await this.setSystemAsTarget(this.player.currentItinerary[1]);
                } else {
     ...
     */
    public async setSystemAsTarget(targetSeed: StarSystemCoordinates) {
        let target = this.getStarSystem()
            .getSystemTargets()
            .find((systemTarget) => starSystemCoordinatesEquals(systemTarget.systemCoordinates, targetSeed));

        if (target === undefined) {
            const newTarget = await this.getStarSystem().addSystemTarget(targetSeed, this.universeBackend);
            if (newTarget !== null) {
                target = newTarget;
