...
export function mountainsLayer(frequency: number, nbOctaves: number, decay: number, lacunarity: number, power: number, minValue: number): elevationFunction {
    ...
    return fractalLayer3D(frequency, nbOctaves, decay, lacunarity, power, minValue, (coords: LVector3, seed: number, gradient: LVector3) => {
        ...
        //const gradientY = 0.5 + LVector3.Dot(gradient.normalizeToNew(), planetNormal);

        //elevation *= gradientY;
        //gradient.scaleInPlace(gradientY);
