...
import { Axis, GizmoManager, Light, LightGizmo, PointLight, Scene, Vector3, WebGPUEngine } from "@babylonjs/core";
...
import {
    ...
    loadMarsNormal,
} from "@/frontend/assets/planetSurfaceTextures/mars";
...
export async function createMarsScene(
    ...
    if (useHighQuality) {
        const albedoResult = await loadMarsHighResolutionAlbedo(scene, engine);
        ...
        albedoBslTexture = {
            type: "texture_2d",
            texture: await loadMarsAlbedo(scene),
        };
    }
    const normal = await loadMarsNormal(scene);
    ...
    progressCallback(1, "Loaded Mars scene");
