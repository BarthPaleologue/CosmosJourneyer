...
import { Vector3, PointLight, AbstractEngine, Scene } from "@babylonjs/core";
...
export async function createGasPlanetScene(
    ...
    const gasPlanetModel = newSeededGasPlanetModel(
        "gasPlanet",
        seed ? Number(seed) : Math.random() * 1000,
        "Gas Planet",
        []
    );
    ...
    const planet = new GasPlanet(gasPlanetModel, ringsLutPool, scene);
