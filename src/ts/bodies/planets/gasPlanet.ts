...
export class GasPlanet extends AbstractBody {
    ...
    constructor(name: string, starSystem: StarSystem, seed: number, parentBodies: IOrbitalBody[]) {
        ...
        this.radius = randRangeInt(Settings.EARTH_RADIUS * 4, Settings.EARTH_RADIUS * 20, this.rng, 33);
        ...
        atmosphere.settings.redWaveLength *= 1 + centeredRand(this.rng, 70) / 6;
        atmosphere.settings.greenWaveLength *= 1 + centeredRand(this.rng, 71) / 6;
        atmosphere.settings.blueWaveLength *= 1 + centeredRand(this.rng, 72) / 6;

        this.postProcesses.atmosphere = atmosphere;

        if (uniformRandBool(0.8, this.rng, 73)) this.createRings();
    }
