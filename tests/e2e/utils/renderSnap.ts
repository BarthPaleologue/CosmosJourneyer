...
export async function renderAndSnap(
    ...
    await page.locator(`#renderer[data-${opts.flagToWait}="1"]`).waitFor({ timeout: 4 * 60 * 1000 });

    await expect(page.locator("#renderer")).toHaveScreenshot(`${opts.shotName}.png`, { timeout: 60_000 });
}
